...
function createMozillaFromCordova(successCB, errorCB, contact) {
    ...
    if (_hasId(contact.id)) {
      ...
      let contacts_rdb = navigator.getRDB('sys/contacts');
      var search = contacts_rdb.query('SELECT * FROM entries WHERE id='+
					contact.id);
      search.onsuccess = function() {
        //moz = search.result[0];
        //In the updated api this returns a cursor so the result is
        // enough
        moz = search.result;
...
function saveContacts(successCB, errorCB, contacts) {
    ...
    while(contact = contacts[i++]){
        var moz = createMozillaFromCordova(function(moz) {
          ...
          let contacts_rdb = navigator.getRDB('sys/contacts'/*,thisprincipal*/);
          var request = contacts_rdb.insert(moz);
...
function remove(successCB, errorCB, ids) {
    ...
    for (var i=0; i < ids.length; i++){
        ...
        search.onsuccess = function() {
            ...
            var request = contacts_rdb.query('DELETE FROM entries WHERE id='+
					     contact.id);
...
function _getAll(successCB, errorCB, params) {
    ...
    let contacts_rdb = navigator.getRDB('sys/contacts');
    var getall = contacts_rdb.query('SELECT * FROM entries);
    var contacts = [];
...
function search(successCB, errorCB, params) {
    ...
    var mozOptions = prefx+' * FROM entries WHERE '+filterBy+' LIKE \''+
                      options.filter + '%\''
    //var request = navigator.mozContacts.find(mozOptions);
    var request = navigator.getRDB().query(mozOptions);
    var contacts = [];
