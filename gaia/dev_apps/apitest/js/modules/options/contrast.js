		...
		render : function() {
			...
			var argumentsObj = {
				onLoadFunc : function (id, params, canvas) {
					...
					$("#slider-holder input").val(AppRegistry.contrast.val);
					$("#slider-holder a").css("left", AppRegistry.contrast.val + "%");
					$("#slider-holder .ui-slider-bg").css("width", AppRegistry.contrast.val + "%");
				},
				params : {}
			};
			...
			$("#slider-holder input").attr("step", "1");
			...
			$("#slider-holder input").on("slidestop", function(event, ui) {
				AppRegistry.contrast.val = parseInt($(this).val(), 10);
