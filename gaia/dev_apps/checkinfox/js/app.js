...
navigator.mozSetMessageHandler('activity', function(activityRequest){
   ...
   activity = activityRequest;
	shared_photo=option.data.blobs[0]
   console.log("got shared photo? " + shared_photo !== undefined);
}.bind(window));
...
var SCREENS={
	...
	,setup: function (screen,data) {
		...
		if (screen==="venue") {
			...
			.then( function (res) {
            console.log("VENUE SCREEN in checkin");
            ...
            if(shared_photo){
               console.log("in shared photo handler");
               var phImage=$("#attachPhotoPlaceholder>img")
                   ,phLabel=$("#attachPhotoPlaceholder>label");

               // Create object url for picked image
               CLIENT.CHECKIN.photoURL=window.URL.createObjectURL(shared_photo);
...
function init() {
	...
	var initialize=function() {
      ...
      // XXX if we recieved a photo via activity set it up here
