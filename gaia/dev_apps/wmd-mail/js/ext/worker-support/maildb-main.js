define(function() {
	...
	this._rdb = null;
        ...
        rdb = navigator.getRDB("wmd-mail");
        rdb.requestOrder = false;
        rdb.onsuccess = function(event) {
            schema = new RDBSchema(emailSchema);
    ...
    MailDB.prototype = {
        ...
        loadHeaderBlock: function(folderId, blockId, callback) {
            ...
            var keyVal = JSON.stringify(folderId + ':' + blockId);
            var where = {type: "=", key: keyVal};
            var cursor = rdb.query(["mail"], ["header"], where, null);
	    cursor.onsuccess = function(event) {
		if (cursor.next()) {
		    callback(JSON.parse(cursor.row.header));
		}
            }
	    ...
	    var req = this._db.transaction(TBL_BODY_BLOCKS, 'readonly')
            ...
            var keyVal = JSON.stringify(folderId + ':' + blockId);
            var where = {type: "=", key: keyVal};
            var cursor = rdb.query(["mail"], ["body"], where, null);
	    cursor.onsuccess = function(event) {
                if (cursor.next()) {
                    callback(JSON.parse(cursor.row.body));
                }
            }

            //req.onerror = this._fatalError;
            //req.onsuccess = function() {
            //    alert("BODY FALLBACK FOR:" + folderId + ':' + blockId);
            //    callback(req.result);
            //};
        },
		...
		var args = pendingRequest.args;
	
                var request = store[type].apply(store, args);

                request.onsuccess = request.onerror = workQueue;
            }

            var mailKeySet = new Set();
            ...
            for (i = 0; i < perFolderStuff.length; i++) {
                ...
                for (var headerBlockId in pfs.headerBlocks) {
                    ...
                    var keyValue = JSON.stringify(pfs.id + ':' + headerBlockId);
                    var headerBlk = pfs.headerBlocks[headerBlockId];
                    mailKeySet.add(keyValue);
                    headerMap.set(keyValue, JSON.stringify(headerBlk));
                ...
                for (var bodyBlockId in pfs.bodyBlocks) {
                    ...
                    var keyValue = JSON.stringify(pfs.id + ':' + bodyBlockId);
                    var bodyBlk = pfs.bodyBlocks[bodyBlockId];
                    mailKeySet.add(keyValue);
                    bodyMap.set(keyValue, JSON.stringify(bodyBlk));
            ...
            for(var [keyVal, valueVal] of mailKeySet.entries()) {
                var headerVal = headerMap.get(keyVal);
                var bodyVal = bodyMap.get(keyVal);
                var obj = {key: keyVal, header: headerVal, body: bodyVal};

                var where = {type: "=", key: keyVal};
                var cursor = rdb.query(["mail"], ["key"], where, null);
		cursor.onsuccess = function(event) {
                    try {
                        if (cursor.next()) { 
                            if(typeof headerVal != 'undefined' && typeof bodyVal != 'undefined') {
                                obj = {header: headerVal, body: bodyVal};
                            } else if(typeof headerVal != 'undefined') {
                                obj = {header: headerVal};
                            } else {
                                obj = {body: bodyVal};
                            }
		            rdb.update(["mail"], where, obj);
                        } else {
                            if(typeof headerVal != 'undefined' && typeof bodyVal != 'undefined') {
                                obj = {key: keyVal, header: headerVal, body: bodyVal};
                            } else if(typeof headerVal != 'undefined') {
                                obj = {key: keyVal, header: headerVal};
                            } else {
                                obj = {key: keyVal, body: bodyVal};
                            }
                            var request = rdb.insert("mail", obj);
                            request.onerror = function() {
                                alert("Failed to insert entry: " + keyVal);
                            }
                        }
                    } catch(err) {
                        if(typeof bodyVal != 'undefined') {
                            var obj = {key: keyVal, body: bodyVal};
                            var request = rdb.insert("mail", obj);
                            request.onsuccess = function() {
	                        //alert("Successfully inserted entry in db");
                            }
 
                            request.onerror = function() {
                                alert("Failed to insert entry in db" + bodyVal);
                            }
                        }
                    }
                }
                ...
                cursor.onerror = function() {
                    ...
                    var obj = {key: keyVal, header: headerMap.get(keyVal), body: bodyMap.get(keyVal)};
                    rdb.insert("mail", obj);
              }
            }
