...
var iac_drive = navigator.connectRDBIAC("https-www.cs.utexas.edu/GDrive");
/*
...
var selectedFileNamesToRecords = {};
...
function init() {
  ...
  $('thumbnails-view-albums-button').onclick = showAlbums;
  ...
  $('thumbnails-album-button').onclick = createAlbumFromSelectedItems;
...
// event handlers.
function initDB() {
  photodb = new RMediaDB('pictures', metadataParserWrapper, {
    version: 2,
...
function getGdriveFiles(){
  var begin = performance.now();
  var req = iac_drive.query(['files'], ['name','content','mimeType'],
                            {type:'=', mimePref:'image'}, null);
  req.onsuccess = function(){
    alert("photos fetched from gdrive: " + (performance.now() - begin));
    var count = 0;
    var objs = [];
    var _date = new Date();
    while(req.next()){
      objs.push({name: req.row.name, content: new Blob([req.row.content]),
                 mimeType: req.row.mimeType, size: req.row.content.size});
    }
    console.log(JSON.stringify(objs));
    
    var next = function(obj){
      if(!obj) return;
      var good = function(metadata){
        obj.date = new Date(_date);
        obj.metadata = metadata;
        obj.type = obj.mimeType;
        files.push(obj);
        thumbnailList.addItem(obj);
        next(objs.shift());
      }
      var bad = function(){
        throw new Error('metadata parsing error');
      }
      var large = function(){
        console.log('parsing big file called');
      }
      metadataParserWrapper(obj.content, good, bad, large);
    }
    next(objs.shift());
  }
  req.onerror = function(){console.log('Something!..but failure');}
}
  ...
  if (enable) {
    ...
    window.navigator.mozNfc.onpeerready = function(event) {
      ...
      if (fileInfo.metadata.video) {
        ...
        // share photo
        if(fileinfo.content) event.peer.sendFile(fileinfo.content);
        else
...
function cropPickedImage(fileinfo) {
  ...
  function gotScripts() {
    if(pickedFileInfo.content) gotFile(pickedFileInfo.content);
    else photodb.getFile(pickedFileInfo.name, gotFile);
  }
...
function updateSelection(thumbnail) {
  ...
  if (selected) {
    ...
    if (files[index].metadata.video) {
      ...
      if(files[index].content){
        selectedFileNamesToBlobs[filename] = files[index].content;
        selectedFileNamesToRecords = files[index];
        selectedFileNamesToRecords = files[index];
      }
      else{
        ...
        photodb.getRecord(filename, function(desc) {
          selectedFileNamesToRecords[filename] = desc;
        });
      }
    }
    if (!isPhone)
  ...
  if (numSelected === 0) {
    ...
    $('thumbnails-album-button').classList.remove('disabled');
  }
}
...
function shareSelectedItems() {
  share(selectedFileNames);
}

function createAlbumFromSelectedItems() {
  var records = selectedFileNames.map(function(name) {
    return selectedFileNamesToRecords[name];
  });
  albumify(records);
}

function albumify(records) {
  loader.load(['js/album.js', 'shared/style/confirm.css', 'style/album.css'],
              function() {
                chooseAlbumAndAdd(records);
              });
};

var album_name_to_obj;

$('view-album-close-button').onclick = function hideFileInformation() {
  // Enable NFC sharing when user closes info and returns to fullscreen view
  setNFCSharing(true);
  $('show-album-view').classList.add('hidden');
};
function passAlbumDescriptor(){
  var desc = album_name_to_obj[this.getAttribute("value")].DESC;
  if(USE_DESC){
    var time = Date.now();
    var token = desc.saveForShare();
    var a = new MozActivity({
      name: 'receive_descriptor',
      data: {
        type: 'album',
        tokens: [token],
        time: time
    ...
    a.onerror = function(e) {
      if (a.error.name === 'NO_PROVIDER') {
        alert("No Reigistered token receivers!");
      }
      else {
    ...
    var time = Date.now();
    photodb.getNames(desc, function(names){
      var blobs = [];
      var done = function(){
...
function showAlbums(){
  function show(albums){
    ...
    albums.forEach(function(obj){
      ...
      album_name_to_obj[obj.name] = obj;
      li.setAttribute("value", obj.name);
      li.onclick = passAlbumDescriptor.bind(li);
      albumlist.appendChild(li);
    });
    $('show-album-view').classList.remove('hidden');
  }
  function err(){
...
function share(names) {
  if(USE_DESC){
    ...
    var errback = function(){
      alert("issue with share!");
    }
    for(var name of names){
      photodb.query_photos({type:'=', name:name}, callback, errback);
    }
  }
  else{
    photodb.getFile(names[0], function(blob){
      var time = Date.now();
      var a = new MozActivity({
        name: 'receive_blob',
        data: {
