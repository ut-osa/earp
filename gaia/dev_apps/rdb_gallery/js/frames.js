...
document.addEventListener('tagged',  function(event){
  files[event.detail.idx] = event.detail.info;
});
...
fullscreenButtons.info.onclick = function() {
  loader.load(['js/info.js', 'shared/style/confirm.css', 'style/info.css'],
              function() {
                showFileInformation(files[currentFileIndex], currentFileIndex);
              });
};
...
function setupFrameContent(n, frame) {
  ...
  var gotfile = function(imagefile) {
    if (fileinfo.metadata.video) {
  ...
  if(fileinfo.content) gotfile(fileinfo.content);
  else photodb.getFile(fileinfo.name, gotfile);
}
