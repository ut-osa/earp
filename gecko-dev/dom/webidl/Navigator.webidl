...
partial interface Navigator {
  [Throws]
  RDB? getRDB(DOMString name);

  [Throws]
  RDBSubsetDesc? getReceivedRDBDesc(unsigned long long token);

  [Throws]
  RDBIAS? registerRDBIAS(DOMString aName);
  [Throws]
  RDBIAC? connectRDBIAC(DOMString aName);
};
