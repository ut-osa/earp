...
'use strict';
function debug(s) { dump("-*- RDBSchema: " + s + "\n"); }

this.EXPORTED_SYMBOLS = ["RDBSchema"];


this.RDBSchema = function(schema) {
  this.schema = schema;
...
// };
this.getSubSchema = function(schema, desc) {
  var res = {};
...
this.RDBSchema.prototype.createSchema = function(rdb, cb) {
...
this.RDBSchema.prototype.getObjectsHelper = function(cursor, res, tab, cb) {
  while (cursor.next()) {
    var obj = {};
...
this.RDBSchema.prototype.getObjects = function(db, tab, options) {
...
this.RDBSchema.prototype.getObject = function(db, tab, cb) {
  if (!db.sync) {
...
this.RDBSchema.prototype.fillObject = function(obj, recursive, cb, trimDescs) {
  ...
  for (var cap in caps) {
    ...
    if (desc.sync) {
      ...
      if (recursive) {
        this.fillObject(capobj, true, trimDescs);
      }
      obj[cap] = capobj;
    } else {
    ...
    if (prop.sync) {
      ...
      if (recursive) {
        for (var i = 0; i < propobj.length; i++) {
          this.fillObject(propobj[i], true, trimDescs);
        }
      }
    } else {
...
this.RDBSchema.prototype.insertObj = function(db, tab, obj) {
  return db.insert(tab, obj);
};

this.RDBSchema.prototype.addProperty = function(obj, proptab, prop, capcols, tokens){
  if(capcols && tokens)
...
this.RDBSchema.prototype.purgeProperties = function(obj, proptab, cb){
  var self = this;
...
this.RDBSchema.prototype.addProperties = function(obj, props, cb){
  var self = this;
  var count = 0;
...
this.RDBSchema.prototype.addPropertiesTx = function(db, obj, props, cb){
  db.startTx();
...
this.RDBSchema.prototype.removeObj = function(obj, tab) {
  return obj.DESC.delete(tab, null);
};

this.RDBSchema.prototype.getToken = function(obj, tab, cb){
...
this.RDBSchema.prototype.insertHelperEntryAndProp = function(db, obj, cb) {
  var req;
  try {
...
this.RDBSchema.prototype.storeTree = function(db, obj, cb) {
  // sync
  if (db.sync) {
    return false;
...
this.RDBSchema.prototype.storeTreeTx = function(db, obj, cb) {
  db.startTx();
...
this.RDBSchema.prototype.queryPathInternal = function(db, tabs, where, orderby, cb, needRoot) {
...
//  }
this.RDBSchema.prototype.queryPath = function(db, tabs, where, orderby, cb) {
...
this.RDBSchema.prototype.enumForest= function(db, tab, recursive, orderby, cb) {
  if (recursive) {
