...
DeviceStorageTypeChecker::Check(const nsAString& aType, nsIDOMBlob* aBlob)
{
  ...
  if (aType.EqualsLiteral(DEVICESTORAGE_APPS) ||
      ...
      StringBeginsWith(aType, NS_ConvertASCIItoUTF16(DEVICESTORAGE_RDB))) {
...
{
  if (StringBeginsWith(aType, NS_ConvertASCIItoUTF16(DEVICESTORAGE_RDB))) {
    aPermissionResult.AssignLiteral("rdb");
    return NS_OK;
  } else if (!aType.EqualsLiteral(DEVICESTORAGE_PICTURES) &&
                                           ...
                                           nsIFile** aFile)
{
  ...
  *aFile = nullptr;
  ...
  if (StringBeginsWith(aStorageType, NS_ConvertASCIItoUTF16(DEVICESTORAGE_RDB))) {
    f = do_CreateInstance(NS_LOCAL_FILE_CONTRACTID);
    f->InitWithPath(StringTail(aStorageType, aStorageType.Length() - DEVICESTORAGE_RDB_LEN));
    f->Create(nsIFile::DIRECTORY_TYPE, 00700);
    f->Clone(aFile);
    return;
  }
...
public:
  ...
  NS_IMETHOD Run()
  {
    ...
    if (mozilla::Preferences::GetBool("device.storage.prompt.testing", false)) {
        //StringBeginsWith(mFile->mStorageType, NS_ConvertASCIItoUTF16(DEVICESTORAGE_RDB))) {
...
// static
void
nsDOMDeviceStorage::CreateDeviceStorageForRDB(nsPIDOMWindow* aWin,
                                           const nsAString &aPath,
                                           nsDOMDeviceStorage** aStore)
{
  ...
  if (NS_FAILED(ds->Init(aWin, NS_ConvertASCIItoUTF16(DEVICESTORAGE_RDB) + aPath, storageName))) {
    *aStore = nullptr;
    return;
  }
                                      ...
                                      bool aEditable, ErrorResult& aRv)
{
  ...
  if (mozilla::Preferences::GetBool("device.storage.prompt.testing", false)) { // ||
      //StringBeginsWith(mStorageType, NS_ConvertASCIItoUTF16(DEVICESTORAGE_RDB))) {
