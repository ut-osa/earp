...
TabChild::RecvDestroy()
{
  ...
  // yxu: clean up RDB connection similarly
  const InfallibleTArray<PRDBChild*>& rdbActors =
    ManagedPRDBChild();
  for (uint32_t i = 0; i < rdbActors.Length(); ++i) {
    static_cast<RDBChild*>(rdbActors[i])->Disconnect();
  }
...
bool
TabChild::DeallocPRDBChild(PRDBChild* aActor)
{
  delete aActor;
  return true;
}
