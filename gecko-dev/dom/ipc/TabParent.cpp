...
TabParent::Destroy()
{
  ...
  // yxu clean up RDB similarly
  const InfallibleTArray<PRDBParent*>& rdbParents =
    ManagedPRDBParent();
  for (uint32_t i = 0; i < rdbParents.Length(); ++i) {
    static_cast<RDBParent*>(rdbParents[i])->Disconnect();
  }
...
PRDBParent*
TabParent::AllocPRDBParent(
                       const nsCString& aGroup,
                       const nsCString& aASCIIOrigin, bool* /* aAllowed */)
{
  return new RDBParent(this);
}

bool
TabParent::DeallocPRDBParent(PRDBParent* aActor)
{
  delete aActor;
  return true;
}
...
bool
TabParent::RecvPRDBConstructor(PRDBParent* aActor,
                               const nsCString& aGroup,
                               const nsCString& aASCIIOrigin,
                               bool* aAllowed)
{
