...
NS_IMPL_CYCLE_COLLECTION_TRAVERSE_BEGIN(Navigator)
  ...
  NS_IMPL_CYCLE_COLLECTION_TRAVERSE(mRDBs)
...
//already_AddRefed<nsDOMRDB>
nsDOMRDB*
Navigator::GetRDB(const nsAString& aName, ErrorResult& aRv)
{
  nsresult rv;
  if (!mWindow || !mWindow->GetOuterWindow() || !mWindow->GetDocShell()) {
    aRv.Throw(NS_ERROR_FAILURE);
  ...
  nsRefPtr<nsDOMRDB> rdb;
  rv = nsDOMRDB::GetRDBFor(mWindow, aName, getter_AddRefs(rdb));
  if (NS_FAILED(rv)) {
    aRv.Throw(rv);
    return nullptr;
  }

  mRDBs.AppendElement(rdb);

  return rdb;
}

already_AddRefed<RDBSubsetDesc>
Navigator::GetReceivedRDBDesc(uint64_t aToken, ErrorResult& aRv)
{
  nsresult rv;
  nsRefPtr<RDBSubsetDesc> d = RDBSubsetDesc::GetFromSharedRepo(aToken);
  if (d) {
    if (!mWindow || !mWindow->GetOuterWindow() || !mWindow->GetDocShell()) {
      aRv.Throw(NS_ERROR_FAILURE);
      return nullptr;
    ...
    nsRefPtr<nsDOMRDB> rdb;
    rv = nsDOMRDB::GetRDBForInternal(mWindow, d->mRDB->mName, getter_AddRefs(rdb), true, false);
    if (NS_FAILED(rv)) {
      aRv.Throw(NS_ERROR_FAILURE);
      return nullptr;
    }
    d = new RDBSubsetDesc(d.get(), rdb);
    //mRDBs.AppendElement(rdb);
  }
  return d.forget();
}
...
RDBIAS*
Navigator::RegisterRDBIAS(const nsAString& aName, ErrorResult& aRv)
{
  RDBIAS* res = RDBIAS::CreateRDBIAS(mWindow, aName);
  return res;
}

already_AddRefed<RDBIAC>
Navigator::ConnectRDBIAC(const nsAString& aName, ErrorResult& aRv)
{
  uint32_t id;
  nsCOMPtr<nsIDocument> doc = mWindow->GetDoc();
  if (!doc)
  ...
  return RDBIAC::CreateRDBIAC(mWindow, aName, id);
}
